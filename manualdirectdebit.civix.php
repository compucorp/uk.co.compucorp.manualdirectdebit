...
/**
 * @return CRM_ManualDirectDebit_Upgrader
 */
function _manualdirectdebit_civix_upgrader() {
  if (!file_exists(__DIR__ . '/CRM/ManualDirectDebit/Upgrader.php')) {
    return NULL;
  }
  else {
    return CRM_ManualDirectDebit_Upgrader_Base::instance();
  }
}
 ...
 * missing them.
 */
function _manualdirectdebit_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _manualdirectdebit_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _manualdirectdebit_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _manualdirectdebit_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
