...
<div style="color: black;">
    ...
    {if $recurringContributionData}
        ...
        <div style="border: 1px solid black; max-width: 600px; width: 100%;">

            <table style="width: 100%; border-bottom: 1px solid black; background: rgb(162,162,162)">
                <tr>
                    <th style="padding-left: 10px; text-align: left;">
                        <p style="color: black;"><strong>{ts}Order Summary{/ts}<strong></p>
                    </th>
                </tr>
            </table>

            {if $priseItemList}
                {foreach from=$priseItemList item=priseItem}
                    <table style="border-collapse: collapse; width: 100%; border-bottom: 1px solid black;">
                        <tr>
                            <td style="border-right: 1px solid black; padding-left: 10px; width: 70%;">
                                <p style="color: black;">{$priseItem.label}</p>
                            </td>
                            <td style="padding-left: 10px; width: 30%;">
                                <p style="color: black;">{$currency}{$priseItem.totalAmount}</p>
                            </td>
                        </tr>
                    </table>
                {/foreach}
            ...
            {if !$priseItemList}
                <table style="border-collapse: collapse; width: 100%; border-bottom: 1px solid black;">
                    <tr>
                        <td style="border-right: 1px solid black; padding-left: 10px; width: 70%;">
                            <p style="color: black;"><strong>{$membershipData.membershipName}</strong></p>
                        </td>
                        <td style="padding-left: 10px; width: 30%;">
                            <p style="color: black;"><strong>{$currency}{$recurringContributionData.total}</strong></p>
                        </td>
                    </tr>
                </table>
            {/if}
            ...
            <table style="border-collapse: collapse; width: 100%; border-bottom: 1px solid black;">
                <tr>
                    <td style="border-right: 1px solid black; padding-left: 10px; width: 70%;">
                        <p style="color: black;"><strong>{ts}Total{/ts}</strong></p>
                    </td>
                    <td style="padding-left: 10px; width: 30%;">
                        <p style="color: black;"><strong>{$currency}{$recurringContributionData.total}</strong></p>
                    </td>
            ...
            <table style="border-collapse: collapse; width: 100%;">
                <tr>
                    <td style="padding-left: 10px;">
                        <p style="color: black">{ts 1=$recurringContributionData.installments 2=$recurringContributionData.installments_paid 3=$currency }To be paid on %1 installments of %3%2 each{/ts}</p>
                    </td>
                </tr>
            </table>
    ...
    {if $mandateData}
        ...
        <div style="max-width: 600px; width: 100%;">
            <p style="color: black;">{ts}Thank you for choosing Direct Debit. Please check that your Direct Debit details below are correct. If they are not, please contact us. If your Direct Debit details are correct, you need do nothing and your Direct Debit will be collected as stated.{/ts}</p>
        </div>
    ...
    {if $membershipData}
        ...
        <div>
            <p style="color: black;">{ts}Your order contains the following membership(s):{/ts}</p>
        </div>
        <!-- End of 3rd section for custom text.  -->

        <!-- Start of code block for generating membership information. Edit with caution. -->
        <div>
            <p style="color: black;">{ts 1=$membershipData.amountPerUnit 2=$membershipData.durationUnit 3=$currency }{$membershipData.membershipName} at %3%1 per %2.{/ts}</p>
        </div>
