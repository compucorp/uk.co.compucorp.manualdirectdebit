...
/**
 * Adaptive Garbage Collection Manager for ManualDirectDebit Extension
 *
 * Implements industry best practices for memory management during direct debit operations
 */
class CRM_ManualDirectDebit_Common_GCManager {
 ...
 */
  ...
  public static function maybeCollectGarbage($operationType = 'default') {
    ...
    // 1. Event-based trigger for token rendering (creates complex object graphs)
    if ($operationType === 'token_rendering') {
      // More frequent for event-based operations
      if ($counters[$operationType] >= 500) {
        $shouldCollect = TRUE;
        $reason = 'event_based';
        $counters[$operationType] = 0;
      }
    ...
    if ($shouldCollect && function_exists('gc_collect_cycles')) {
      ...
      if ($cycles > 0 || $memoryFreed > (5 * 1024 * 1024)) {
        \Civi::log()->debug(sprintf(
          'ManualDirectDebit GC (%s): %d cycles, %.2fms, %s freed, memory: %s, interval: %d',
          $reason,
          $cycles,
          $duration,
